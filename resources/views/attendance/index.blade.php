        ...
        <div class="overflow-x-auto">
            ...
            <table class="w-full border-collapse text-sm">
                ...
                <tbody class="text-center divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $loop->iteration % 2 == 0 ? 'bg-gray-50/30' : '' }}">
                        <td class="border border-gray-300 px-4 py-3 font-medium text-gray-900">{{ $date }}</td>
                        @foreach($groupOrder as $program => $groups)
                            @foreach($groups as $groupName)
                                ...
                                @endphp
                                <td class="border border-gray-300 px-2 py-3 font-bold {{ $record && $record->percentage >= 80 ? 'text-green-600' : 'text-red-600' }}">
                                    @if($record)
                                        {{ number_format($record->percentage, 0) }}%
                                    @else
