...
<div class="space-y-6 max-w-2xl mx-auto">
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">{{ __('admin.add_new_user') }}</h1>
        ...
        <form action="{{ route('admin.users.store') }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" name="name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#3dab8c] focus:ring-[#3dab8c] sm:text-sm" value="{{ old('name') }}" required>
                    @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">{{ __('admin.email_address') }}</label>
                    <input type="email" name="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#3dab8c] focus:ring-[#3dab8c] sm:text-sm" value="{{ old('email') }}" required>
                    @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">{{ __('admin.mobile_number') }}</label>
                    <input type="text" name="mobile" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#3dab8c] focus:ring-[#3dab8c] sm:text-sm" value="{{ old('mobile') }}" required>
                ...
                <div>
                    ...
                    <select name="role" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#3dab8c] focus:ring-[#3dab8c] sm:text-sm" required>
                        <option value="3" {{ old('role') == 3 ? 'selected' : '' }}>{{ __('admin.viewer_desc') }}</option>
                        <option value="2" {{ old('role') == 2 ? 'selected' : '' }}>{{ __('admin.editor_desc') }}</option>
                        <option value="1" {{ old('role') == 1 ? 'selected' : '' }}>{{ __('admin.administrator_desc') }}</option>
                    </select>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">{{ __('admin.password') }}</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700">{{ __('admin.confirm_password') }}</label>
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full bg-[#3dab8c] text-white px-4 py-2 rounded-md shadow hover:bg-[#349479] font-bold transition-colors">
                        {{ __('admin.create_administrative_user') }}
                    </button>
                </div>
            </div>
        </form>
