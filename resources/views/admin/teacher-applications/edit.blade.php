...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">{{ __('admin.edit_teacher_application') }}</h1>
        <a href="{{ route('admin.teacher-applications.index') }}" class="text-gray-500 hover:text-gray-700">{{ __('admin.back_to_list') }}</a>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
             ...
             <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('admin.mobile') }}</label>
                <div class="mt-1 p-2 w-full bg-gray-50 border border-gray-300 rounded-md">{{ $teacherApplication->mobile }}</div>
            </div>
             <div>
                <label class="block text-sm font-medium text-gray-700">{{ __('admin.designation') }}</label>
        ...
        <form action="{{ route('admin.teacher-applications.update', $teacherApplication->id) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status" class="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-[#3dab8c] focus:border-[#3dab8c] sm:text-sm rounded-md">
                    <option value="0" {{ $teacherApplication->status == 0 ? 'selected' : '' }}>{{ __('admin.pending') }}</option>
                    <option value="1" {{ $teacherApplication->status == 1 ? 'selected' : '' }}>{{ __('admin.approved') }}</option>
                    <option value="2" {{ $teacherApplication->status == 2 ? 'selected' : '' }}>{{ __('admin.rejected') }}</option>
                </select>
            </div>
