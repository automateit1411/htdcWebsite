                    ...
                    <tr class="hover:bg-gray-50 transition-colors" x-show="search === '' || '{{ strtolower(str_replace(["'", "\\"], ["\'", "\\\\"], $notice->title)) }}'.includes(search.toLowerCase())">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            @if($notice->file_path)
                                ...
                                @endphp
                                <a href="{{ asset('storage/' . $notice->file_path) }}" target="_blank" class="text-blue-600 hover:text-blue-900 flex items-center gap-1">
                                    @if($isImage)
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <a href="{{ route('admin.notices.edit', $notice->id) }}" class="text-[#3dab8c] hover:text-[#0d3a37] mr-3 font-semibold">Edit</a>
                            <form action="{{ route('admin.notices.destroy', $notice->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure?');">
                                @csrf
                                @method('DELETE')
