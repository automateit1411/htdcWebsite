    ...
    <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100 mb-8">
        ...
        <form action="{{ route('admin.custom-page2.store-item', $custom_page2->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('admin.item_title') }}</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('admin.image_label') }}</label>
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">{{ __('admin.file_pdf_doc') }}</label>
            ...
            <button type="submit" class="px-6 py-3 bg-gradient-to-r from-[#3dab8c] to-[#0d3a37] text-white rounded-xl font-bold shadow-lg hover:shadow-2xl transition-all">{{ __('admin.add_item') }}</button>
        </form>
    </div>

    {{-- Existing Items --}}
    <div class="bg-white rounded-2xl shadow-xl p-8 border border-gray-100">
        <h2 class="text-xl font-bold text-gray-800 mb-6">{{ __('admin.existing_items') }}</h2>
        @if($items->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                @foreach($items as $item)
                    <div class="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden group hover:shadow-lg transition-all">
                        ...
                        @endif
                        <div class="p-4">
                            <h3 class="font-bold text-gray-800">{{ $item->title ?? __('admin.untitled') }}</h3>
                            @if($item->description)
                                <p class="text-sm text-gray-500 mt-1 line-clamp-2">{{ $item->description }}</p>
                            @endif
                            <div class="flex items-center gap-2 mt-3">
                                @if($item->file_path)
                                    <a href="{{ asset('storage/' . $item->file_path) }}" target="_blank" class="text-xs bg-blue-500 text-white px-3 py-1 rounded-lg font-bold hover:bg-blue-600 transition">{{ __('admin.file') }}</a>
                                @endif
                                <form action="{{ route('admin.custom-page2.destroy-item', [$custom_page2->id, $item->id]) }}" method="POST" onsubmit="return confirm('{{ __('admin.delete_item_confirm') }}')">
                                    @csrf @method('DELETE')
                                    <button class="text-xs bg-red-500 text-white px-3 py-1 rounded-lg font-bold hover:bg-red-600 transition">{{ __('admin.delete') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
