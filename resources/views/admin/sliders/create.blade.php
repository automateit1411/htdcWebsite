...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">{{ __('admin.create_new_slider') }}</h1>
            <p class="text-gray-600 mt-1">{{ __('admin.create_slider_desc') }}</p>
        </div>
    ...
    <form action="{{ route('admin.sliders.store') }}" method="POST" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 space-y-6">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.select_gallery_image') }}</label>
                
                <!-- Selected Image Preview -->
                <div id="selectedImagePreview" class="hidden mb-4">
                    ...
                    <p class="mt-2 text-sm text-gray-600 flex items-center gap-1">
                        ...
                        {{ __('admin.image_selected') }} <span id="selectedImageName" class="font-medium"></span>
                    </p>
                </div>
                ...
                <button type="button" onclick="openImageSelectorModal()" 
                    ...
                    {{ __('admin.select_image_from_gallery') }}
                </button>
                ...
                @enderror
                <p class="mt-2 text-xs text-gray-500">{{ __('admin.click_to_select_image') }}</p>
            </div>

            <!-- Slider Title -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.slider_title') }}</label>
                <input type="text" name="title" value="{{ old('title') }}" 
                       placeholder="{{ __('admin.slider_title_placeholder') }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#3dab8c] focus:border-[#3dab8c]">
                @error('title')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                <p class="mt-2 text-xs text-gray-500">{{ __('admin.slider_title_desc') }}</p>
            </div>

            <!-- Order -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('admin.display_order') }}</label>
                ...
                @error('order')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
                <p class="mt-2 text-xs text-gray-500">{{ __('admin.display_order_desc') }}</p>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <label for="is_active" class="text-sm font-medium text-gray-700">{{ __('admin.active') }}</label>
                <p class="text-xs text-gray-500">{{ __('admin.check_to_display_homepage') }}</p>
            </div>
        </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-2xl shadow-2xl max-w-6xl w-full mx-4">
            ...
            <div class="flex justify-between items-center p-6 border-b">
                <h3 class="text-2xl font-bold text-gray-800">{{ __('admin.select_gallery_image') }}</h3>
                    ...
                    @foreach($galleries as $gallery)
                        <div onclick="selectImage({{ $gallery->id }}, '{{ addslashes($gallery->title ?? 'Image #' . $gallery->id) }}', '{{ asset('storage/' . $gallery->image) }}')"
                            ...
                            <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all flex items-center justify-center">
                                <span class="text-white opacity-0 group-hover:opacity-100 font-medium">{{ __('admin.click_to_select') }}</span>
                            </div>
...
function confirmImageSelection() {
    if (!tempSelectedImage) {
        alert('{{ __('admin.please_select_image') }}');
        return;
    }
