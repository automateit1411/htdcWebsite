...
<div class="space-y-6">
    <div class="flex justify-between items-center">
        <h1 class="text-2xl font-bold text-gray-800">{{ __('admin.create_form_download') }}</h1>
        <a href="{{ route('admin.form-downloads.index') }}" class="text-gray-500 hover:text-gray-700">{{ __('admin.back_to_list') }}</a>
    </div>

    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form action="{{ route('admin.form-downloads.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <ul class="text-xs text-blue-800 space-y-1">
                    ...
                    <li>• <strong>{{ __('admin.new') }}</strong>: form-downloads created within the last 7 days</li>
                    <li>• <strong>{{ __('admin.old') }}</strong>: form-downloads older than 7 days</li>
                    <li>• A "New" badge with animated GIF will appear on the website for new form-downloads</li>
                </ul>
            </div>
