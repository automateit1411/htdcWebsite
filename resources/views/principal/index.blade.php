...
@section('title', 'Principal List - Hazera-Taju Degree College')
        ...
        <div x-data="{
                ...
                init() {
                    fetch('/proxy/employees/principal')
                        .then(response => {
                            if (!response.ok) throw new Error({{ __('website.failed_fetch_principal_data') }});
                            return response.json();
                        })
                        .then(data => this.principals = data)
                        .catch(err => this.error = err.message || {{ __('website.unable_to_load_principals') }})
            ...
            <div class="flex items-center gap-3 mb-4">
                <h1 class="text-xl font-bold text-gray-800">{{ __('website.principal_info') }}</h1>
            ...
            <template x-if="loading">
                <div class="flex items-center justify-center py-20">
                    <div class="text-gray-500 text-sm">{{ __('website.loading_principals') }}</div>
                </div>
            </template>
                ...
                <div>
                                    ...
                                    <a :href="`/principle-list/${principal.employeeCode || index}`"
                                        ...
                                        <div class="flex justify-center mb-3">
                                            <img :src="principal.profileScan || 'https://via.placeholder.com/150'"
                                                alt="{{ __('website.principal_photo') }}"
                                        ...
                                        <h2 class="text-sm font-bold text-gray-800 text-center leading-tight"
                                            x-text="principal.name || {{ __('website.no_name') }}"></h2>
                                        <p class="text-xs text-gray-400 text-center mt-0.5" x-text="principal.banglaName || ''"></p>
                                        <p class="text-xs text-[#3dab8c] text-center font-medium mt-1.5" x-text="principal.designation_name || {{ __('website.na') }}"></p>
                                    </a>
                                </template>
                            </div>
                        </div>
                    ...
                    <template x-if="principals.length === 0">
                        <div class="text-center py-20">
                            <p class="text-gray-400 text-sm">{{ __('website.no_principals') }}</p>
                        </div>
                    </template>
                </div>
            </template>
