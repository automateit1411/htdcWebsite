        ...
        <div x-data="{
                ...
                init() {
                    fetch('/proxy/employees/vice-principal')
                        ...
                        .catch(err => this.error = err.message || 'Unable to load vice-principals')
            ...
            <div class="flex items-center gap-3 mb-4">
                <h1 class="text-xl font-bold text-gray-800">Vice-Principal Information</h1>
            ...
            <template x-if="loading">
                <div class="flex items-center justify-center py-20">
                    <div class="text-gray-500 text-sm">Loading vice-principals...</div>
                </div>
            </template>
                ...
                <div>
                                ...
                                <template x-for="(vp, index) in vicePrincipals" :key="index">
                                    <a :href="`/vice-principle-list/${vp.employeeCode || index}`"
                                        ...
                                        <h2 class="text-sm font-bold text-gray-800 text-center leading-tight"
                                            x-text="vp.name || 'No name'"></h2>
                                        <p class="text-xs text-gray-400 text-center mt-0.5" x-text="vp.banglaName || ''"></p>
                                        <p class="text-xs text-[#3dab8c] text-center font-medium mt-1.5" x-text="vp.designation_name || 'N/A'"></p>
                                    </a>
                                </template>
                            </div>
                        </div>
                    ...
                    <template x-if="vicePrincipals.length === 0">
                        <div class="text-center py-20">
                            <p class="text-gray-400 text-sm">No vice-principals found.</p>
                        </div>
                    </template>
                </div>
            </template>
