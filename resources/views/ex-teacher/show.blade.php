...
@section('title', 'Ex-Teacher - Hazera-Taju Degree College')

@section('content')
    <div class="p-4 max-w-5xl mx-auto">
        <a href="{{ url('/ex-teacher-list') }}"
            class="inline-flex items-center gap-1 text-sm text-[#0d3a37] hover:text-green-700 mb-4 font-medium">&larr; {{ __('website.back_to_ex_teacher_list') }}</a>
        ...
        <div x-data="{
                ...
                init() {
                    fetch('/proxy/employees/ex-teacher')
                        .then(response => {
                            if (!response.ok) throw new Error('{{ __('website.failed_fetch_ex_teacher') }}');
                        ...
                        .then(data => {
                            ...
                            if (!found) {
                                this.error = '{{ __('website.ex_teacher_not_found') }}';
                            } else {
                                this.teacher = found;
                            }
                        })
                        .catch(err => this.error = err.message || '{{ __('website.unable_to_load_ex_teacher') }}')
            ...
            <template x-if="loading">
                <div class="flex items-center justify-center py-20">
                    <div class="text-gray-500 text-sm">{{ __('website.loading_ex_teacher_details') }}</div>
                </div>
            </template>
