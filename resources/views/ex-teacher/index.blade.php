...
@section('title', 'Ex-Teachers - Hazera-Taju Degree College')
        ...
        <div x-data="{
                ...
                init() {
                    fetch('/proxy/employees/ex-teacher')
                        .then(response => {
                            if (!response.ok) throw new Error('{{ __('website.failed_fetch_ex_teacher_data') }}');
                            return response.json();
                        })
                        .then(data => this.exTeachers = data)
                        .catch(err => this.error = err.message || '{{ __('website.unable_to_load_ex_teachers') }}')
            ...
            <div class="flex items-center gap-3 mb-4">
                <h1 class="text-xl font-bold text-gray-800">{{ __('website.ex_teacher_info') }}</h1>
            ...
            <template x-if="loading">
                <div class="flex items-center justify-center py-20">
                    <div class="text-gray-500 text-sm">{{ __('website.loading_ex_teachers') }}</div>
                </div>
            </template>
                ...
                <div>
                                    ...
                                    <a :href="`/ex-teacher-list/${teacher.employeeCode || index}`"
                                        ...
                                        <div class="flex justify-center mb-3">
                                            <img :src="teacher.profileScan || 'https://via.placeholder.com/150'"
                                                alt="{{ __('website.ex_teacher_photo') }}"
                    ...
                    <template x-if="exTeachers.length === 0">
                        <div class="text-center py-20">
                            <p class="text-gray-400 text-sm">{{ __('website.no_ex_teachers') }}</p>
                        </div>
                    </template>
                </div>
            </template>
